...
const segments = computed<Segment[]>(() => {
    ...
    for (const b of blocks) {
        if (b.start > cursor)
            out.push({ type: "md", md: props.body.slice(cursor, b.start) });
        if (b.kind === "card" && b.data)
            out.push({ type: "card", card: b.data as Card });
        else if (b.kind === "drill" && b.data)
            out.push({ type: "drill", drill: b.data as Drill });
        cursor = b.end;
    }
    if (cursor < props.body.length)
        out.push({ type: "md", md: props.body.slice(cursor) });
    return out;
});
    ...
    <div class="md-render" @click="onClick">
        <template v-for="(seg, i) in segments" :key="i">
            ...
            >
                ...
                <div v-if="seg.card!.hint" class="card-hint">
                    подсказка: {{ seg.card!.hint }}
                </div>
                <template v-if="seg.card!.tags" #footer>
                    <NTag
                        v-for="t in seg.card!.tags"
                        :key="t"
                        size="small"
